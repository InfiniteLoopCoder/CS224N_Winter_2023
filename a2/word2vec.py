...
def naiveSoftmaxLossAndGradient(
    ...
    prediction = softmax(np.dot(outsideVectors, centerWordVec)) # shape: (num words in vocab, )
    truth = np.zeros_like(prediction)  
    truth[outsideWordIdx] = 1
    loss = -np.log(prediction[outsideWordIdx])
    gradCenterVec = np.dot(outsideVectors.T, prediction - truth) # shape: (word vector length, )
    gradOutsideVecs = np.outer(prediction - truth, centerWordVec)
...
def negSamplingLossAndGradient(
    ...
    ## Create word embeds
    allWord = outsideVectors[indices] # shape: (K+1, word vector length)
    allWord[1:] *= -1 # reverse all neg samples
    loss = -1 * np.sum(np.log(sigmoid(np.dot(allWord, centerWordVec))))

    ## Calculate gradients
    middleSig = sigmoid(np.dot(allWord, centerWordVec)) # shape: (K+1, )
    gradCenterVec = np.dot(middleSig - 1, allWord) # shape: (word vector length, )

    ## Noting there might be same words is VERY IMPORTANT!
    gradOutsideVecs = np.zeros_like(outsideVectors) # shape: (num words in vocab, word vector length)
    for i, idx in enumerate(negSampleWordIndices):
        gradOutsideVecs[idx] += (1 - middleSig[i + 1]) * centerWordVec # shape: (word vector length, )
    gradOutsideVecs[outsideWordIdx] += (middleSig[0] - 1) * centerWordVec # shape: (word vector length, )
    ### END YOUR CODE
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for outsideWord in outsideWords:
        curLoss, curGradCenterVecs, curGradOutsideVectors = word2vecLossAndGradient(centerWordVectors[word2Ind[currentCenterWord]], word2Ind[outsideWord], outsideVectors, dataset)
        loss += curLoss
        gradCenterVecs[word2Ind[currentCenterWord]] += curGradCenterVecs
        gradOutsideVectors += curGradOutsideVectors
