...
class ParserModel(nn.Module):
    ...
    def embedding_lookup(self, w):
        ...
        selected_emb = torch.index_select(self.embeddings, 0, w.flatten())
        x = selected_emb.view(w.shape[0], -1)
        ### END YOUR CODE
        return x
    ...
    def forward(self, w):
        ...
        mid = F.relu(torch.matmul(emb, self.embed_to_hidden_weight) + self.embed_to_hidden_bias)
        mid = self.dropout(mid)
        logits = torch.matmul(mid, self.hidden_to_logits_weight) + self.hidden_to_logits_bias
